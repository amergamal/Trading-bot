...
def ensure_database_schema():
    """Ensure TickerRange and ohlc tables exist."""
    with db_lock:
...
def calculate_pmh_pml_and_insert(ticker):
    ...
    Uses ohlc_5min table. Inserts a record with default values (pmh=1, pml=1, range='1%') if no pre-market data.
    """
    with db_lock:
        conn = sqlite3.connect('EOD_data.db')
        cursor = conn.cursor()
        try:
            ...
            if exists:
                logger.debug(f"TickerRange record already exists for {ticker} on {today_date_sql}, checking for update")
            else:
                # Insert a default record to ensure ticker is added
                cursor.execute("""
            ...
            if pmh and pml and pmh != 0 and pml != 0:
                ...
                # Update the existing record (whether default or previously updated)
                cursor.execute("""
                    UPDATE TickerRange
                    SET pmh = ?, pml = ?, range = ?
...
def process_ticker(symbol, start_time_str):
    try:
        ...
        # Insert ticker into TickerRange immediately
        calculate_pmh_pml_and_insert(symbol)
        ...
        send_command(sock, f'UNSB {symbol} MINCHART')
        sock.close()
