...
class StrategyLogic:
    def __init__(self, position_add, risk_management, socketio=None, trading_end=time(13, 0)):  # risk_management parameter is optional
        ...
        self.socketio = socketio or SocketIO()  # Use passed socketio or fallback
        ...
        self.socketio = socketio  # Use the passed Flask-SocketIO instance
    ...
    def fetch_tickers_from_db(self):
        ...
        now = tm.time()
        if self.ticker_cache is not None and (now - self.cache_time) < self.cache_duration:
            self.logger.debug("Returning cached tickers")
        ...
        for attempt in range(retries):
            try:
                ...
                # Add new tickers to self.tickers
        ...
        self.logger.error(f"Failed to fetch tickers after {retries} attempts.")
        self.ticker_cache = []
        self.cache_time = now
    ...
    def add_ticker(self, ticker, rsi_1m, rsi_5m):
        with self.lock:
            ...
            if ticker in self.tickers:
                self.logger.debug(f"Ticker {ticker} already added, updating data")
                self.tickers[ticker].update({
                    'rsi_1m': rsi_1m,
                    'rsi_5m': rsi_5m,
                    'active_trades': self.tickers[ticker].get('active_trades', {})
                })
                return
    ...
    def fire_signals(self, ticker, entry_price, hod, shares, strategy_type, risk, s_loss):
        try:
            ...
            time_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # Default to now as string
            ...
            if self.insert_trade_signal(signal):
                self.logger.info(f"Signal {signal['trade_id']} inserted into TradeSignal.")
                if self.socketio:
                    self.socketio.emit('trade_signal_update', signal)  # Push to frontend
                    self.logger.info(f"Emitted trade_signal_update: {signal}")
                self.risk_management.receive_signal(signal)
            else:
                self.logger.error(f"Failed to insert signal {signal['trade_id']}.")
            return signal
        except Exception as e:
    ...
    def run_ticker(self, ticker, strategy_type):
        ...
        try:
            while datetime.now().time() <= self.trading_end:
                ...
                if strategy_type in ['1Min', '1Minco', '1Minde'] and current_time.second == 4:
...
if __name__ == "__main__":
    ...
    try:
        while True:
            time.sleep(1)  # Keep the main thread running to prevent the script from exiting
