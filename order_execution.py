...
def send_target_order(ticker, shares, trade_id, strategy, stop_price, target_price, token_map):
    """
    Send a short sell order with stop loss and target buy-back using a SCRIPT command.
    Returns tokens for the short sell, stop loss, and buy-back orders.
    """
    
    stop_token = generate_token()
    buy_token = generate_token()
    
    # Validate unique tokens
    if len({stop_token, buy_token}) != 2:
        raise ValueError("Tokens must be unique: stop_token={}, buy_token={}".format(stop_token, buy_token))

    # Initialize token_map for each order
    with token_map_lock:
        
        token_map[stop_token] = {'order_status': None, 'order_act_action': None, 'trade_id': trade_id, 'strategy': strategy, 'order_type': 'stop'}
        token_map[buy_token] = {'order_status': None, 'order_act_action': None, 'trade_id': trade_id, 'strategy': strategy, 'order_type': 'buy'}

    
    # Construct the SCRIPT command using STOPTYPE:RANGE for simplicity
    command_target = (
        f"SCRIPT ANUBIS Symbol={ticker};"
        f"OCO=STOP STOPTYPE:MARKET StopPrice:{stop_price} ACT:BUY QTY:{shares} TIF:DAY+;LIMIT Price:{target_price} ACT:BUY QTY:{shares} TIF:DAY+;"
        f"BUY=Send Token={stop_token} Token={buy_token}"
    )
    
    # Log the command for debugging
    logging.debug(f"Sending DAS command: {command_target}")

    # Send the command and start the response listener
    client_socket = send_command(command_target)
    response_thread = threading.Thread(target=listen_for_responses, args=(client_socket, token_map))
    response_thread.start()
    return stop_token, buy_token
...
class OrderExecution:
    ...
    def execute_command(self, command):
        ...
        entry_price = command.get('price')  # Retrieve entry price for limit or target orders
        
        # Log the details of the order
        if order_type == 'limit' and entry_price is not None:
            entry_price = round(float(entry_price), 2)
            stop_price = round(float(stop_price), 2)
            self.logger.info(f"Processing limit order for {ticker} with {shares} shares at entry price {entry_price}")
        elif order_type == 'target' and entry_price is not None:
            entry_price = round(float(entry_price), 2)
            stop_price = round(float(stop_price), 2)
            target_price = round(entry_price * 0.85, 2)
            
            self.logger.info(f"Processing target order for {ticker} with {shares} shares at entry price {entry_price}, stop price {stop_price}, target price {target_price}")
        else:
            self.logger.info(f"Processing {order_type} order for {ticker} with {shares} shares")

        # Check BorrowedShares table for available shares (for sell and target orders)
        if order_type in ['limit', 'market', 'target']:
            ...
            if available_shares < shares:
                ...
                self.logger.info(f"Requesting to borrow {needed_shares} additional shares for {ticker}.")
                ...
                self.logger.info("Waiting 5 seconds for Slocate to process the borrow request.")
                time.sleep(5)

                if borrow_result['status'] == 'error':
                    self.logger.warning(f"Borrow failed for {ticker}: {borrow_result['message']}")
        ...
        if order_type == 'marketl':
            ...
            self.logger.info(f"Sent buy market order for {ticker} with token {buy_token}")
            ...
            if order_confirmed:
                self.logger.info(f'Buy market order executed for {ticker}')
                if stop_price:
                    stop_token = send_stop_sell_market_order(ticker, shares, stop_price, trade_id, strategy, self.token_map)
                    self.logger.info(f'Sent stop sell market order for {ticker} at stop price {stop_price}')
                    ...
                    if stop_order_confirmed:
                        ...
                        self.logger.error(f"Stop sell market order for {ticker} failed.")
            else:
                self.logger.error(f"Buy market order for {ticker} failed.")
            return    
        ...
        if order_type == 'limit':
            ...
            self.logger.info(f"Order details for {ticker} with trade ID {trade_id} stored successfully.")
            ...
            if order_confirmed:
                ...
                self.logger.error(f"Sell limit order for {ticker} failed.")
        ...
        if order_type == 'target':
            ...
            if order_confirmed:
                self.logger.info(f'Sell market order executed for {ticker}')
                if not entry_price:
                    self.logger.error("Entry price required for target order.")
                    return
            
                stop_token, buy_token = send_target_order(ticker, shares, trade_id, strategy, stop_price, target_price, self.token_map)
                self.logger.info(f"Sent target order for {ticker} with stop token {stop_token}, buy token {buy_token}")

                stop_order_confirmed = self.wait_for_stop_order_confirmation(stop_token)
                with token_map_lock:  # Ensure thread-safe access
                    self.store_order_details(stop_token, 'StopMarket')
                buy_order_confirmed = self.wait_for_stop_order_confirmation(buy_token)
                with token_map_lock:  # Ensure thread-safe access
                    self.store_order_details(buy_token, 'BuyMarket')
                ...
                if stop_order_confirmed and buy_order_confirmed:
                        
                    self.logger.info(f'Target order (short sell) executed for {ticker}')
                    ...
                    trade_details = {
                        ...
                        'ticker': ticker,
                        'shares': shares,
                        'entry_price': self.token_map[sell_token].get('price', entry_price),
                        'stop_loss': stop_price,
                        'target_price': target_price,
                        'sellOrderID': self.token_map[sell_token].get('order_id', 'N/A'),
                        'stopOrderID': self.token_map[stop_token].get('order_id', 'N/A'),
                        #'targetOrderID': self.token_map[buy_token].get('order_id', 'N/A')
                    }
                    self.logger.info(f"Sending trade details to TradeMonitor: {trade_details}")
                    self.trade_monitor.receive_order_details(trade_details)
                    self.logger.info("Trade details sent to TradeMonitor successfully")
                    ...
                    self.logger.error(f"Stop loss or buy-back order for {ticker} failed to confirm.")
                     
            else:
                self.logger.error(f"Target order (short sell) for {ticker} failed.")
    ...
    def get_available_shares(self, ticker):
        ...
        try:
            ...
            if row:
                ...
                self.logger.info(f"{available_shares} shares available for {ticker}.")
    ...
    def wait_for_order_confirmation(self, token, timeout=30):
        ...
        while True:
            ...
            if time.time() - start_time > timeout:
                self.logger.warning(f"Order {token} confirmation timed out.")
...
if __name__ == "__main__":
    ...
    # Example command for testing the target order
    test_command = {
        'order_type': 'target',
        'ticker': 'TSLA',
        'shares': 1,
        ...
        'strategy': 'SCALP',
        'stop_price': 294.00,
        'price': 293.80  # Entry price
    }
    order_execution.execute_command(test_command)
