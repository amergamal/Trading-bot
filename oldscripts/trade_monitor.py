...
class TradeMonitor:
    ...
    def insert_active_trade(self, trade_details):
        ...
        # Add to SharesActivity table
        self.add_shares_activity(trade_details)
    ...
    def add_shares_activity(self, trade_details):
        """Add a record to SharesActivity when a trade is received."""
        conn = self.get_db_connection()
        if not conn:
            return
        try:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO SharesActivity (trade_id, ticker, strategy_name, shares_used, shares_outstanding)
                VALUES (?, ?, ?, ?, ?)""",
                (trade_details['tradeID'], trade_details['ticker'], trade_details['strategy'], trade_details['shares'], trade_details['shares'])
            )
            conn.commit()

            # Subtract the shares used from BorrowedShares table
            cursor.execute("""
                UPDATE BorrowedShares
                SET available_shares = available_shares - ?
                WHERE ticker = ?""",
                (trade_details['shares'], trade_details['ticker'])
            )
            conn.commit()
            self.logger.info(f"Added record to SharesActivity and updated BorrowedShares for {trade_details['ticker']} with {trade_details['shares']} shares.")
        except sqlite3.Error as e:
            self.logger.error(f"Error adding shares activity: {e}")
        finally:
            conn.close()
    ...
    def check_lu_price(self, trade_id, ticker, shares, stop_loss, lu_price, stop_order_id):
        ...
        if lu_price is not None and lu_price < stop_loss:
            new_stop_price = lu_price - 0.05
            self.logger.info(f"LU price {lu_price} for {ticker} is below stop loss {stop_loss}. Sending replace order to {new_stop_price} for trade {trade_id}")
    ...
    def process_replace_response(self, response, trade_details):
        ...
        try:
            ...
            self.store_replace_order(trade_id, time, ticker, shares, new_stop_price, order_id, action, status, status)
            if status == 'Replaced':
                self.update_stop_loss_in_active_trades(trade_id, ticker, shares, new_stop_price)
                       
            elif status == 'ReplaceRej':
    ...
    def update_stop_loss_in_active_trades(self, trade_id, ticker, shares, new_stop_price):
        """Update the stop loss price in ActiveTrades to the new stop price after replacement."""
        conn = self.get_db_connection()
        if not conn:
            return
        try:
            cursor = conn.cursor()
            cursor.execute("""
                UPDATE ActiveTrades SET stop_loss = ? 
                WHERE tradeID = ? AND ticker = ? AND shares = ?""",
                           (new_stop_price, trade_id, ticker, shares)
                           )
            conn.commit()
            self.logger.info(
                f"Updated stop loss price in ActiveTrades for trade {trade_id}, ticker {ticker} to {new_stop_price}")
        except sqlite3.Error as e:
