...
class CandleFetch:
    ...
    def fetch_bars(self, symbol, start_time, end_time, timeframe):
        logging.info(f'Fetching {timeframe} data for {symbol} from {start_time} to {end_time}')
        try:
    ...
    def main(self):
        ...
        if not tickers:
            logging.error('No tickers found for the specified date.')
            return
        ...
        while True:
            ...
            print(f"Next 1-min run time: {next_run_time_1min}, Next 5-min run time: {next_run_time_5min}")
            
            while datetime.now() < next_run_time_1min and datetime.now() < next_run_time_5min:
                time.sleep(1)

            if datetime.now() >= next_run_time_1min:
                print(f"Fetching 1-min bars at {datetime.now()}")
                all_1min_data = []
                ...
                if all_1min_data:
                    ...
                    print("1-min data:")
                    print(combined_1min_data)
                    self.insert_into_db('ohlc_1min', combined_1min_data.values.tolist())
                else:
                    logging.info("No 1-minute data fetched for any tickers.")
                time.sleep(2)
                next_run_time_1min = self.get_next_minute_interval(1)

            if datetime.now() >= next_run_time_5min:
                print(f"Fetching 5-min bars at {datetime.now()}")
                all_5min_data = []
                for ticker in tickers:
                ...
                if all_5min_data:
                    ...
                    print("5-min data:")
                    print(combined_5min_data)
                    self.insert_into_db('ohlc_5min', combined_5min_data.values.tolist())
                else:
                    logging.info("No 5-minute data fetched for any tickers.")
                time.sleep(2)
                next_run_time_5min = self.get_next_five_minute_interval()
