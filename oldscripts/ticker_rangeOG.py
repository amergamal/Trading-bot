...
def request_previous_day_close(sock, ticker):
    ...
    if response:
        ...
        for line in lines:
            if line.startswith("$Bar") and ticker in line:
                ...
                logging.info(f"Previous day close for {ticker} on {start_date}: {previous_close}")
    ...
    if previous_close is None:
        logging.warning(f"Previous day close for {ticker} could not be retrieved on {start_date}")
...
def update_PCL(ticker, pcl):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            logging.warning(f"No matching record found for {ticker} on {today_date} to update PCL.")
        else:
            logging.info(f"Updated PCL: {pcl} for {ticker} on {today_date}")
    except sqlite3.Error as e:
        logging.error(f"Error updating PCL for {ticker} on {today_date}: {e}")
    finally:
        conn.commit()
        conn.close()
...
def parse_and_store_data(response, symbol):
    ...
    for line in lines:
        ...
        if line.startswith('$Bar'):
            ...
            if len(parts) < 8 or parts[-1] != '5':  # Skip if it's not 5-minute data
                logging.warning(f"Ignoring non-5-minute data for {symbol}.")
                continue
    ...
    if data:
        ...
        logging.info(f'Data for {symbol} successfully inserted into ohlc_PM5min table.')
...
def calculate_pmh_pml_and_insert(ticker):
    ...
    if pmh and pml:
        ...
        if exists:
            ...
            logging.info(f"Updated PMH: {pmh}, PML: {pml}, Range: {formatted_percentage} for {ticker} on {today_date}")
        else:
            # Insert a new record
            cursor.execute("""
            ...
            logging.info(f"Inserted PMH: {pmh}, PML: {pml}, Range: {formatted_percentage} for {ticker} on {today_date}")
    else:
        logging.warning(f"No pre-market data available for {ticker} to calculate PMH and PML.")
    ...
    if result:
        try:
            ...
            if result:
                ...
                logging.info(f"Updated GAP for {ticker}: {gap}%")
            else:
                logging.warning(f"No 09:30 or earlier close found in ohlc_PM5min for {ticker}.")
        except ValueError as e:
            logging.error(f"Error in calculating gap for {ticker}. Ensure numeric values: {e}")        
    else:
        logging.warning(f"Previous close not found in TradeParameters for {ticker}.")
...
def process_ticker(symbol, start_time_str, end_time_str):
    ...
    try:
        ...
        logging.info(f'Initial data collection for {symbol} completed. Calling calculate_pmh_pml_and_insert.')
...
def monitor_new_tickers(start_time_str, end_time_str):
    """
    ...
    while True:
        ...
        for ticker in tickers:
            if ticker not in processed_tickers:
                logging.info(f"New ticker found: {ticker}. Starting data collection.")
        ...
        for ticker in list(processed_tickers):  # Iterate over a copy to safely modify the set
            if ticker not in tickers:
                logging.info(f'{ticker} has been removed from the database. Stopping its data collection.')
        ...
        if now.hour == 9 and now.minute == 30:
            logging.info("9:30 AM reached. Re-fetching and processing all tickers.")
...
def main():
    ...
    logging.info(f"Starting ticker monitoring and data collection from {start_time_str} until {end_time_str}.")
    
    # Monitor for new tickers
    monitor_thread = Thread(target=monitor_new_tickers, args=(start_time_str, end_time_str))
    monitor_thread.start()
    
    # Ensure the main thread waits for monitoring to finish (which it never will)
    monitor_thread.join()
