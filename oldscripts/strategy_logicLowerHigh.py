...
class StrategyLogic:
    ...
    def process_market_data(self, ticker, strategy_type):
        ...
        if strategy_type in ['1Min', '1MinBS']:
            table_name = 'ohlc_1min'
        elif strategy_type in ['5MinBS']:
    ...
    def condition_backside_short(self, data, index, vwap):
        """ 
        Condition for backside short: 1 green candle followed by 2 red candles
        - The highs of both red candles must be below the high of the initial green candle.
        - The second red candle’s high must be lower than the first red candle’s high.
        - **New Condition:** The current price must be above VWAP.
        """
        if index < 2:
            return False, None  # Not enough data

        green_candle = data.iloc[index - 2]
        red_candle1 = data.iloc[index - 1]
        red_candle2 = data.iloc[index]

        # Check candle colors
        if (
            green_candle["close"] > green_candle["open"] and  # Green candle
            red_candle1["close"] < red_candle1["open"] and  # First red candle
            red_candle2["close"] < red_candle2["open"] and  # Second red candle
            red_candle1["high"] < green_candle["high"] and  # First red candle high below green
            red_candle2["high"] < red_candle1["high"] and  # Second red candle high below first red
            red_candle2["close"] > vwap  # **New condition: Price must be above VWAP**
        ):
            return True, green_candle["high"]  # Return stop loss at green candle high
    ...
    def fire_auto_signals(self, data, ticker, strategy_type):
        try:
            ...
            if active_trade_status in ["open", "pending"]:  # Skip if a trade is active
                self.logger.info(f"Skipping signal generation for {ticker} due to active trade status: {active_trade_status}")
            ...
            for i in range(1, len(data)):
                ...
                try:
                 ...
                 # VWAP for the current row (rounded to 2 decimals)
                try:
                    vwap = round(row['vwap'], 2)
                    
                except KeyError as e:
                    self.logger.error(f"Missing 'vwap' column in data for {ticker}: {e}")
                    continue
                ...
                try:
                    ...
                    if strategy_type in ["1MinBS", "5MinBS"]:
                        backside_signal, stop_loss = self.condition_backside_short(data, i, vwap)
                        if backside_signal:
                            self.logger.info(f"Backside short condition met for {ticker} at row {i}. Signal generated.")
                            signal = self.generate_signal(row, ticker, strategy_type, vwap, stop_loss)
    ...
    def run_ticker(self, ticker, strategy_type):
        ...
        try:
            while datetime.now().time() <= self.trading_end:
                ...
                if strategy_type in ['1Min', '1MinBS'] and current_time.second == 4 and not self.pause_1min_strategy:
                ...
                if strategy_type in ['5MinBS'] and current_time.minute % 5 == 0 and current_time.second == 4:
