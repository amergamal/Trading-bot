...
class EndOfDay:
    def __init__(self, socketio=None, eod_time="15:55"):
    ...
    def end_of_day_tasks(self):
        schedule.every().day.at(self.eod_time).do(self.close_positions)
        while True:
    ...
    def handle_response(self, response):
        ...
        for line in lines:
            parts = line.split()
            if len(parts) < 3:
                self.logger.warning(f"Invalid response line: {line}")
                continue
    ...
    def move_trade_to_closed(self, trade_id, ticker, exit_price, exit_time):
        ...
        try:
            ...
            if trade:
                ...
                # Update BorrowedShares (adding back shares to available_shares)
