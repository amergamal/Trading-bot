...
class SLMonitor:
        ...
        for line in lines:
            if line.startswith('%ORDER'):
                ...
                try:
                    ...
                    self.update_stop_loss_in_active_trades(order_id, price)
                    
                    self.update_stop_market_status(order_id, status, price, update_status_time)
                except IndexError as e:
                    self.logger.error(f'Error parsing %OrderAct response: {e}')
                continue    
                
                    
    def update_stop_market_status(self, order_id, status, price, update_status_time):
        ...
        try:
            ...
            cursor.execute("""
               UPDATE StopMarket
               SET status = ?, act_status = ?, price = ?, time = ?
               WHERE orderID = ?""",
               (status, status, price, update_status_time, order_id)
            )
            conn.commit()
    ...
    def update_borrowed_shares(self, ticker, shares):
        """Add the shares back to available_shares in BorrowedShares table when an order is executed."""
        conn = self.get_db_connection()
        if not conn:
            return
        try:
        ...
        with self.lock:
            ...
            try:
                ...
                if trade:
                    ...
                    # Add shares back to BorrowedShares table
                    self.update_borrowed_shares(ticker, shares)
