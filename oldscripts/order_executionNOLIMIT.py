...
class OrderExecution:
    def __init__(self, risk_management=None, position_add=None):
        self.risk_management = risk_management
        self.sock = None
        self.connected = False
        self.response_buffer = []
        self.token_map = {}
        self.trade_monitor = TradeMonitor()  # Create an instance of TradeMonitor
    ...
    def execute_command(self, command):
        ...
        if order_type == 'add':
            ...
            return True  # Exit after handling the 'add' order type    
        ...
        while True:
            ...
            with token_map_lock:
                ...
                if order_status == 'Executed':
                    ...
                    print(f"Order {token} status: Executed - Returning True")  # Print the value
                    return True
                if order_act_status == 'Send_Rej':
                    logging.info(f"Order {token} status: Send_Rej")
                    print(f"Order {token} status: Send_Rej - Returning False")  # Print the value
                    return False
            
            if time.time() - start_time > timeout:
                logging.warning(f"Order {token} confirmation timed out.")
                print(f"Order {token} confirmation timed out - Returning False")  # Print the value
                return False
