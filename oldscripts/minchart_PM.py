...
def parse_and_store_data(response, symbol):
    ...
    if data:
        ...
        logger.info(f'Data for {symbol} successfully inserted into ohlc_1min table.')
    ...
    try:
        ...
        while True:
            ...
            if current_time >= end_time:
                logger.info(f'Duration of {run_duration} seconds reached for {symbol}, stopping data collection.')
                break
        ...
        # Start new threads for any new tickers found in the database
            for symbol in tickers:
                if symbol not in processed_tickers:
                    logger.info(f'Found new ticker {symbol}, starting data collection.')
            ...
            time.sleep(60)  # Check for new tickers every minute

def main(strategy_logic=None):
    if strategy_logic is None:
        logger.error("No strategy_logic provided, cannot fetch tickers")
        return
    logger.info("Starting minchart_PM data collection.")
    ...
    try:
        ...
        logger.error(f"Error in minchart_PM main: {e}")
