...
class Slocate:
    def __init__(self, socketio=None):
        ...
        self.accepted_orders = {}  # Dictionary: trade_id -> set of order_ids
        self.socketio = socketio
        self.current_ticker = None  # Initialize to avoid AttributeError
    ...
    def handle_response(self, response):
        ...
        for line in lines:
            ...
            if len(parts) > 1 and parts[0].upper() == "%SLORDER":
                ...
                order_id = parts[1]
                ticker = parts[2]
                self.logger.info(f"SLORDER response - Order ID: {order_id}, Ticker: {ticker}, Status: {status}")
                ...
                if status == "Offered":
                    ...
                    if self.current_ticker and ticker != self.current_ticker:
                        self.logger.debug(f"Ignoring offer for {ticker}; expected {self.current_ticker}")
                        continue
    ...
    def accept_offer(self, offer, trade_id):
        ...
        # Initialize set for this trade_id if not exists
    ...
    def reset_offers(self, ticker):
        """Clear offers for a specific ticker before processing a new request."""
    ...
    def clear_accepted_orders(self, trade_id):
        """Clear accepted orders for a specific trade_id."""
    ...
    def receive_order_details(self, ticker, shares, trade_id):
        ...
        # Reset offers for this ticker to ensure no stale offers are used
