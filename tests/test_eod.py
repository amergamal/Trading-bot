...
class EndOfDay:
    ...
    def end_of_day_tasks(self):
        ...
        while True:
            ...
            if current_time.hour == 18 and current_time.minute == 17:  # Adjust time as needed
    ...
    def process_buy_response(self, line, trade_id, ticker, shares, token):
        ...
        if parts[0].upper() == "%ORDER":
            ...
            ticker = parts[4]
            shares = parts[5]
            price = float(parts[6]) if parts[6].isdigit() else 0.0  # Handle price correctly
...
if __name__ == "__main__":
    ...
    listener_thread = threading.Thread(target=end_of_day._listen_to_server, daemon=True)
