...
class StrategyLogic:
    ...
    def process_market_data(self, ticker):
        self.logger.info(f"Fetching 1-minute data for {ticker}")
        data_1min = self.get_data_from_db(ticker, 'ohlc_1min')
        self.logger.info(f"Fetching 5-minute data for {ticker}")
    ...
    def run_ticker(self, ticker):
        ...
        while datetime.now().time() <= self.trading_end:
            self.logger.info(f"Processing market data for {ticker}")
