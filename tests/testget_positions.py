...
def send_get_positions():
    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
            ...
            # Construct the get positions order command
            
            command = f'GET POSITIONS'
            

            # Send the command to the server
            logging.info(f"Sending get positions command: {command.strip()}")
            client_socket.sendall(command.encode())

            # Close the connection after sending
            logging.info("Get positions command sent, closing connection.")
