...
class APIConnection:
    ...
    def receive_response(self):
        if self.sock:
            try:
                ...
                # Filter out $Quote messages entirely (no logging or processing)
                if "$Quote" in response:
                    return None  # Ignore quote messages
    ...
    def continuously_receive(self):
        while self.connected:
            ...
            if response:
                # Only handle non-quote responses
    ...
    def start_local_server(self):
        ...
        self.logger.info(f'Local server started on port {LOCAL_SERVER_PORT}')
    ...
    def handle_client(self, client_socket, addr):
        try:
            while True:
                ...
                if command:
                    self.logger.info(f'Received command from client: {command}')
