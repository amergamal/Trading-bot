...
class VwapFetch:
    def __init__(self, db_path, trade_monitor):
        ...
        # Store initial quote data for high_price and quote_time
        self.initial_quote_data = {}  # {ticker: {'high_price': float, 'quote_time': str}}
        ...
        while True:
            ...
            if login_response:
                if 'LOGIN SUCCESSED' in login_response:
                    ...
                    self.update_account_equity()  # Fetch account equity once
                    break
    ...
    def update_quote_data(self, ticker, last_price=None, high_price=None, quote_time=None, current_date=None):
        """Update TradeParameters with LAST, HIGH, HI_TIME."""
        with self.db_lock:
            conn = self.connect_to_db()
            cursor = conn.cursor()
            query = """
    ...
    def handle_quote_data(self, quote_data):
        """Handle and update LAST, HIGH, HI_TIME based on the quote data."""
        ...
        # Initialize or update initial quote data for high_price and quote_time
        if ticker not in self.initial_quote_data:
            ...
            # Update high_price and quote_time only if new high_price is higher
        ...
        # Use stored values for high_price and quote_time
